...
template <typename T> class _box {
  T value;
...
public:
  ...
  void
  reset(T &&t) noexcept(noexcept(value = hidden::_move_assign_if_noexcept(t))) {
    value = hidden::_move_assign_if_noexcept(t);
  }
};

template <typename T> class _box<T &> {
...
public:
  ...
  _box(TT &&t,
       GG &&guard) noexcept(noexcept(static_cast<T &>(static_cast<TT &&>(t))))
...
class [[nodiscard]] basic_scope_exit : Policy {
  ...
  using _ctor_from =
      std::is_constructible<detail::_box<EF>, EFP, detail::_empty_scope_exit>;
  template <typename EFP>
#ifndef _MSC_VER
  using _noexcept_ctor_from = std::bool_constant<noexcept(
      detail::_box<EF>(std::declval<EFP>(), detail::_empty_scope_exit{}))>;
#else
  // MSVC thinks that it is a function style cast
  using _noexcept_ctor_from =
      std::bool_constant<noexcept(detail::_box<EF>::_box(
          std::declval<EFP>(), detail::_empty_scope_exit{}))>;
#endif
public:
  ...
  basic_scope_exit(basic_scope_exit &&that) noexcept(
      noexcept(detail::_box<EF>(that.exit_function.move(), that)))
...
template <typename R, typename D> class unique_resource {
  ...
  template <typename RR, typename DD,
            typename = std::enable_if_t<
                std::is_constructible_v<detail::_box<R>, RR,
                                        detail::_empty_scope_exit> &&
                std::is_constructible_v<detail::_box<D>, DD,
                                        detail::_empty_scope_exit>>>
  unique_resource(RR &&r, DD &&d, bool should_run) noexcept(
      noexcept(detail::_box<R>(std::forward<RR>(r),
                               detail::_empty_scope_exit{})) &&
  ...
  friend struct detail::hidden::factory_holder; // a level of indirection is the
                                                // trick...
public:
  template <typename RR, typename DD,
            typename = std::enable_if_t<
                std::is_constructible<detail::_box<R>, RR,
                                      detail::_empty_scope_exit>::value &&
                std::is_constructible<detail::_box<D>, DD,
                                      detail::_empty_scope_exit>::value>>
  ...
  unique_resource(unique_resource &&that) noexcept(
      noexcept(detail::_box<R>(that.resource.move(),
                               detail::_empty_scope_exit{})) &&
  ...
  // THIS IS NOT A POINTER TYPE, the following operations are only available if
  // R is a native pointer
...
struct factory_holder {
  ...
  static auto special_maker(MR &&r, MD &&d, bool shouldrun) {
    unique_resource<std::decay_t<MR>, std::decay_t<MD>> resource{
        std::forward<MR>(r), std::forward<MD>(d), shouldrun};
    return resource;
  }
};
} // namespace hidden
...
[[nodiscard]]
auto make_unique_resource_checked(MR &&r, const S &invalid, MD &&d) noexcept(
    std::is_nothrow_constructible_v<std::decay_t<MR>, MR> &&
  ...
  auto resource = detail::hidden::factory_holder::special_maker(
      std::forward<MR>(r), std::forward<MD>(d), !mustrelease);
