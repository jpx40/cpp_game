...
TitleScene::~TitleScene() { UnloadTexture(texture); }
...
void TitleScene::draw() {
  ...
  auto draw_with_backdrop = [](const string &text, int x, int y, int fontSize,
                               Color color, Color backdropColor) {
    DrawText(text.c_str(), x + 1, y + 1, fontSize, backdropColor);
    DrawText(text.c_str(), x, y, fontSize, color);
  };

  draw_with_backdrop("Press 'space' to play!", 10, 10, 30, GOLD, BLACK);
  ...
  DrawText(text.c_str(), GameConstants::WorldWidth / 2 - text_width / 2,
           texture_y + texture.height + 20, 20, BLACK);

  int mouseX = GetMouseX() / ScreenScale;
  int mouseY = GetMouseY() / ScreenScale;

  int rectSize = GameConstants::WorldWidth / 20;
  DrawRectangle(mouseX - rectSize / 2, mouseY - rectSize / 2, rectSize,
                rectSize, DARKPURPLE);
